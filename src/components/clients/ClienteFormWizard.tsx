...
import { ClientDocumentsPhase } from "./ClientDocumentsPhase";
import { decideGovCredential } from "@/lib/govCredential";
...
export default function ClienteFormWizard({
  ...
  // Acesso GOV.BR (opcional, só no cadastro). NÃO entra em ClientFormValues nem
  // no save_client — a credencial trafega pela RPC auditada save_gov_credential
  // (cifra server-side, senha revelada só por reveal_gov_credential com log).
  ...
  async function handleConfirm() {
    ...
    if (mode === "edit") { setSaving(false); toast.success("Cliente atualizado!"); onSaved?.(id, form.full_name); return; }

    toast.success("Cliente cadastrado!");

    // Acesso GOV.BR (best-effort): reusa a RPC auditada save_gov_credential
    // (cifra server-side, exige consentimento). Uma falha aqui NÃO desfaz o
    // cadastro — o cliente já existe; a credencial pode ser posta na aba Gov.br.
    const govDecision = decideGovCredential(govUser, govPass, govConsent);
    if (govDecision === "save") {
      const { error: govErr } = await (supabase as unknown as {
        rpc: (fn: string, args: Record<string, unknown>) => Promise<{ error: { code?: string; message?: string } | null }>;
      }).rpc("save_gov_credential", {
        p_client_id: id,
    ...
    } else if (govDecision === "missing-consent") {
      toast.warning("Cliente cadastrado. A credencial GOV.BR não foi salva: marque o consentimento para guardá-la (pode fazer isso na aba Gov.br).");
    }
  ...
  function renderClassificacao() {
    ...
    return (
      <>
        ...
        {/* Acesso GOV.BR (opcional) — só no cadastro. A credencial é cifrada e
            só revelada com registro em auditoria; a edição fica na aba Gov.br. */}
        {mode === "create" && (
          <>
            <div className="cli-formsec" style={{ gridColumn: "1 / -1" }}>Acesso GOV.BR (opcional)</div>
            <div>
              <label className="cli-label">Usuário GOV.BR (CPF/login)</label>
              <input className="cli-input" value={govUser} autoComplete="off"
                onChange={e => setGovUser(e.target.value)} placeholder="CPF ou login do GOV.BR" />
            </div>
            <div>
              <label className="cli-label">Senha GOV.BR</label>
              <input className="cli-input" type="password" value={govPass} autoComplete="new-password"
                onChange={e => setGovPass(e.target.value)} placeholder="•••••••" />
            </div>
            <div style={{ gridColumn: "1 / -1" }}>
              <label className="cli-wa-check" style={{ alignItems: "flex-start" }}>
                <input type="checkbox" checked={govConsent} onChange={e => setGovConsent(e.target.checked)} />
                <span>O cliente <strong>consente</strong> com a custódia segura da credencial GOV.BR (termo v1.0). Necessário para guardar a senha.</span>
              </label>
              <span className="cli-cep-hint" style={{ display: "block", marginTop: 4 }}>
                Campos opcionais. A senha é gravada cifrada e só é revelada com registro em auditoria.
              </span>
            </div>
          </>
        )}
