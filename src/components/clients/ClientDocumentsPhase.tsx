...
import { runCooperadoOnboarding, fetchCooperadoChecklist, type CooperadoOnboardingResult } from "@/lib/cooperadoOnboarding";
...
export function ClientDocumentsPhase({ clientId, clientName, userId, onDone }: {
  ...
  // Recarrega apenas os status do checklist após um upload. NÃO regenera os
  // documentos de sistema — a geração roda uma única vez, na montagem. Regenerar
  // aqui a cada upload era o que multiplicava as linhas (bug 2026-07-22).
  async function reloadChecklist() {
    const checklist = await fetchCooperadoChecklist(clientId);
    setRes((prev) => (prev ? { ...prev, checklist } : prev));
  }
