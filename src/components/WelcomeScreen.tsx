...
export default function WelcomeScreen({ onDismiss }: { onDismiss: () => void }) {
  ...
  const shortcuts = [
    { icon: "💬", label: "Chat IA", action: onDismiss },
    { icon: "📋", label: "Filas de Tarefas", action: onDismiss },
    { icon: "👥", label: "Clientes", action: () => navigate("/clientes") },
    ...
    <div style={{ padding: "24px", maxWidth: 900, margin: "0 auto", overflowY: "auto", height: "100%" }}>
      ...
      {(summary.overdue > 0 || summary.critical > 0) && (
        <div style={{
          ...
          <p style={{ fontWeight: 600, color: "#ef4444", margin: 0, fontSize: 15 }}>⚠️ Atenção Necessária</p>
          <div style={{ display: "flex", gap: 20, marginTop: 8 }}>
            {summary.overdue > 0 && (
              <span style={{ color: "#fca5a5", fontSize: 13 }}>🔴 {summary.overdue} tarefa(s) atrasada(s)</span>
            )}
            {summary.critical > 0 && (
              <span style={{ color: "#fbbf24", fontSize: 13 }}>🚨 {summary.critical} tarefa(s) crítica(s)</span>
            )}
            {summary.dueSoon > 0 && (
              <span style={{ color: "#93c5fd", fontSize: 13 }}>⏳ {summary.dueSoon} prazo(s) próximo(s)</span>
            )}
          </div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 12, marginBottom: 24 }}>
        {[
          { label: "Total de Tarefas", value: summary.total, icon: "📋", color: "#c9a84c" },
          { label: "Pendentes", value: summary.pending, icon: "⏳", color: "#f59e0b" },
          { label: "Em Andamento", value: summary.inProgress, icon: "🔄", color: "#3b82f6" },
          { label: "Críticas", value: summary.critical, icon: "🚨", color: "#ef4444" },
        ].map((kpi) => (
      ...
      {/* Quick Shortcuts */}
      <div style={{ marginBottom: 24 }}>
        <h3 style={{ fontSize: 14, fontWeight: 600, color: "#c9a84c", marginBottom: 10 }}>⚡ Atalhos Rápidos</h3>
          ...
          {shortcuts.map((s) => (
            <button
              ...
              style={{
                ...
                color: "#e2d5a0", cursor: "pointer",
                fontSize: 13, fontWeight: 500,
                transition: "all 0.2s",
                display: "flex", alignItems: "center", gap: 6,
              }}
              onMouseOver={(e) => { e.currentTarget.style.background = "rgba(201,168,76,0.18)"; e.currentTarget.style.borderColor = "rgba(201,168,76,0.4)"; }}
      ...
      {recentClients.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <h3 style={{ fontSize: 14, fontWeight: 600, color: "#c9a84c", marginBottom: 10 }}>👥 Últimos Clientes</h3>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {recentClients.map((c, i) => (
              <div key={i} style={{
                ...
                <span style={{ color: "#e2d5a0", fontSize: 13 }}>👤 {c.full_name}</span>
