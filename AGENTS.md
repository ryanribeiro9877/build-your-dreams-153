...
- **Item 5 — peça não persistia (fechado no complemento de 07/08).** O diagnóstico final mostrou que o banco já tinha o dono correto: `salvar_peca` cria `client_documents(status='pendente')` + tarefa `Revisar peça`, e `get_revisao_peca_context`/`decidir_revisao_peca` fecham o ciclo. O elo ausente era materializar o texto. O pipeline principal em `validating_n1` agora usa `pecaDocument.ts` para gerar **DOCX editável** (A4, Arial 12, corpo justificado), sobe em `client-documents` como `minuta` e chama `salvar_peca` sob o JWT do redator. O path é determinístico por run e o upload atômico (`upsert:false`) impede duas tentativas concorrentes de criarem o mesmo arquivo; upload sem linha é removido e tarefa de confecção intermediária é cancelada se a RPC falhar. A confecção é vínculo opcional — advogados sem `can_assign` continuam arquivando a peça e criando a revisão humana. Checklists mecânicos ficam no chat e são removidos do DOCX. Cliente/processo vêm somente do contexto resolvido da sessão; sem cliente inequívoco, não grava no dossiê errado e a resposta declara o motivo. O `.md` antigo do glue multi-hop também passou a DOCX. Testes: `pecaDocument.test.ts` abre o OOXML e prova conteúdo, além dos testes da nota de falha em `resultadoDaAcao.test.ts`.
...
**Validação do complemento da peça:** 319 testes Deno + 459 vitest + `deno check` do orchestrator + `vite build`. `tsc` segue com o único erro pré-existente de `useMenuAccess.ts`; o lint global segue com o débito histórico do repositório (nenhum diagnóstico novo nos arquivos alterados).
