// supabase/functions/gerar-kit-documental/fillDocxTemplate.ts
//
// PORTE Deno (Onda 2.3) de src/lib/fillDocxTemplate.ts — MESMA lógica, byte a
// byte, exceto o import do JSZip (bare "jszip" → esm.sh, para o runtime Deno do
// Supabase Edge). fillDocumentXml continua uma função PURA (regex sobre o XML),
// idêntica à do front; não há divergência de comportamento entre a geração pela
// tela e a geração pelo chat.
//
// Preenchedor DETERMINÍSTICO de templates .docx (mail-merge de placeholders):
// carrega o template com JSZip, mexe SÓ nas partes de texto (word/document.xml +
// headers/footers) e reempacota — header, estilos, marca d'água e mídia ficam
// INTACTOS. NENHUM modelo de linguagem escreve ou reformula o corpo jurídico — é
// substituição pura {{placeholder}} → valor exato do cadastro. Dado ausente NUNCA
// vira lacuna silenciosa: vira "[A PREENCHER: <chave>]" e volta em `missing`.

// ?no-dts: o .d.ts do jszip usa `export = JSZip` (CJS) e o type-checker do Deno
// não sintetiza o default; o build JS do esm.sh SEMPRE expõe `export default`,
// então em runtime `import JSZip from ...` funciona. Sem os tipos, JSZip é `any`
// (aceitável: a superfície usada aqui é mínima e coberta por engine.test.ts).
