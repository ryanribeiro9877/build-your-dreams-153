-- Caminho A (max_tokens=20000): peças completas legítimas levam ~330-400s. O limite
-- do watchdog sobe para 380s (acima do tempo de geração, abaixo dos 400s de wall-clock).
-- O N3 renova orchestration_runs.updated_at durante o streaming, então um run ativo
-- nunca é marcado como travado mesmo perto do teto. Idempotente.

CREATE OR REPLACE FUNCTION public.fail_stale_orchestration_runs(
  p_max_age interval DEFAULT interval '380 seconds'
) RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
